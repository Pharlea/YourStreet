...
const statusLabel: Record<string, string> = {
  ...
  completed: "Concluída",
  waiting_reopen_confirmation: "Aguardando reabertura",
};
...
export function MapView() {
  ...
  const loadOccurrences = async () => {
    try {
      ...
      setOccurrences(data);
      // Buscar coordenadas para cada ocorrência com endereço
      const coordsMap = new Map<number, [number, number]>();
      for (const occ of data) {
        if (occ.address) {
          const coords = await getCoordinates(occ.address);
          if (coords) {
            coordsMap.set(occ.id, coords);
          } else {
            // Fallback para coordenadas simuladas se geocoding falhar
            coordsMap.set(occ.id, getOccurrenceCoordinates(occ.id));
          }
        } else {
          // Sem endereço, usar simuladas
          coordsMap.set(occ.id, getOccurrenceCoordinates(occ.id));
        }
  ...
  useEffect(() => {
    ...
    filteredOccurrences.forEach((occurrence) => {
      const coords = coordinates.get(occurrence.id) || getOccurrenceCoordinates(occurrence.id);
  ...
  }, [filteredOccurrences, coordinates, navigate]);
